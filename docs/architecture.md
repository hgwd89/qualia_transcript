...
- `services/storage_paths.py`: shared output/upload path validation, including symlink and Windows reparse/junction rejection below managed roots, plus OS-level ancestry-pinned reads and exclusive-create writes.
- `services/managed_write_commit_guard.py`: exact-generation re-pinning across generated-output/media metadata DB commits, post-commit namespace verification, and pinned-generation cleanup.
...
Transcription uses the same ancestry-pinned read boundary for uploaded media, but pathname-only decoders and providers are never handed the mutable managed upload path. `create_media_read_snapshot()` copies the exact already-opened media object into an owner-private temporary file outside `UPLOAD_DIR`, verifies source generation and byte size across the copy, and `services/transcription.py` passes only that private snapshot pathname to PyAV, OpenAI, and faster-whisper. Both transcription provider paths close the snapshot in function-level `finally` blocks, so success and failure remove the temporary copy. OpenAI and local faster-whisper success paths both persist immutable provider text under `raw_transcripts` before the canonical `done` commit; the local path concatenates the model-emitted segment text without stripping or reformatting it, and refuses a successful transition when the provider output is empty. The durable job lease callback is propagated into local execution, including OpenAI-to-local fallback, and is checked before local status mutation/inference and again before local evidence creation. Error handling also revalidates the lease after rollback, so a recovered stale worker cannot overwrite the recovered attempt with a late `error` transition. If long-form OpenAI fails after earlier chunks have committed, OpenAI-to-local fallback first deletes only that transcription's partial canonical `Segment` rows under the durable write/lease fence; immutable raw OpenAI chunk snapshots remain retained as audit evidence while the local provider rebuilds the canonical Segment set from scratch.

Immutable raw transcript evidence uses a separate retained-source boundary under `OUTPUT_DIR/raw_transcripts`. `write_raw_snapshot_json()` creates a UUID-suffixed JSON file through the ancestry-pinned exclusive-create path and fsyncs the written bytes before returning. `read_raw_snapshot_batch()` retains one verified raw-directory identity across enumeration and every snapshot read: POSIX enumerates and opens relative to a pinned directory descriptor with `O_NOFOLLOW` plus generation checks. Windows retains the original raw-directory handle, records its volume/file-index identity, rejects reparse points, revalidates that the public pathname still names the same directory after enumeration and after each final-file acquisition, and verifies that the pathname generation still matches the already-opened file before consuming bytes. Windows may permit a directory rename despite the retained metadata handle; when that occurs, replacement is detected and the batch fails closed rather than accepting successor bytes. Project-deletion provenance hashing and production-readiness validation consume this batch instead of glob/check/reopen pathnames. A storage-safety failure aborts deletion provenance staging and is surfaced as an explicit readiness defect.

Managed writes use two linked fences. `open_managed_file_for_create()` first creates a UUID-named destination only once and returns the already-opened binary stream; media upload and every generated-report writer write to that stream instead of reopening `target.full_path`. On POSIX the final file is created descriptor-relative beneath the pinned parent with `O_CREAT|O_EXCL|O_NOFOLLOW`; on Windows ancestor handles deny write/delete sharing while the final file is opened with `CREATE_NEW` and `FILE_FLAG_OPEN_REPARSE_POINT`. After the writer closes, `services/managed_write_commit_guard.py` re-pins the exact final file generation and its ancestor chain for the complete database metadata commit window. The public managed pathname is verified before and after commit. If POSIX renames an acquired ancestor during commit, post-commit verification fails, the just-created `GeneratedFile` or `MediaFile` row is compensatingly deleted, and cleanup unlinks only the pinned predecessor generation through the original parent descriptor. On Windows the retained handles omit delete sharing, so the same ancestor rename is rejected while the commit is in progress. Replacement successors are never selected for rollback deletion.

The download route streams from the already-opened handle and reconstructs HTTP metadata from its `fstat()` result. `Content-Length`, `Last-Modified`, ETag conditional handling, and byte-range `206` responses are preserved without reopening the pathname. Legacy `GeneratedFile` rows whose nullable `original_filename` is unset fall back to the stored basename. Resolver-based registration and other reads still reject linked/reparse entries, but they do not claim the stronger ancestry-pinned guarantee unless they use the open-handle boundary explicitly. Filesystem and database operations still cannot form one native transaction; the commit guard, post-commit namespace verification, compensating metadata deletion, UUID-only basenames, and pinned-generation rollback cleanup provide the application-level consistency boundary.
