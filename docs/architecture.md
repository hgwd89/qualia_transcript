...
- `services/managed_storage_write.py`: OS-level ancestry-pinned creation/deletion for managed generated outputs; POSIX uses descriptor-relative exclusive no-follow creation and Windows retains non-reparse directory handles while creating the final file with `CREATE_NEW`.
...
Formal generated-output writes use the symmetric managed write boundary rather than saving Word/Excel/CSV libraries directly to a pathname. `open_managed_file_for_write()` pins the ancestor chain before the final file exists; POSIX creates the UUID basename relative to the pinned parent with `O_CREAT|O_EXCL|O_NOFOLLOW`, while Windows retains non-reparse directory handles and creates the final regular file with `CREATE_NEW`. The report writer receives that already-opened binary stream. `write_and_register_generated_file()` keeps the exact file handle and ancestor pins live through writer completion, flush/fsync, POSIX generation-identity verification against the logical managed path, and `GeneratedFile` DB commit. On Windows the retained directory/final handles deny write/delete sharing for that lifetime, so namespace replacement is prevented without a conflicting self-reopen. Writer or commit failure deletes only the pinned partial object; it never follows a subsequently replaced project-directory pathname for cleanup. The older split `write_output_target()` / `register_generated_file()` helpers remain compatibility-only and are not used by formal report generators.

The download route streams from the already-opened handle and reconstructs HTTP metadata from its `fstat()` result. `Content-Length`, `Last-Modified`, ETag conditional handling, and byte-range `206` responses are preserved without reopening the pathname. Legacy `GeneratedFile` rows whose nullable `original_filename` is unset fall back to the stored basename. Filesystem and database operations still cannot form one transaction; UUID-only basenames plus the combined pinned write/register boundary prevent output generation from being redirected outside the acquired managed directory and keep rollback cleanup bound to the exact partial file.
