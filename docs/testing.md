...
The same required category is run by `.github/workflows/safe-check.yml` as the Windows `safe-smoke` job and by `scripts/check_all.ps1` with no flags. CI also runs a focused Ubuntu `posix-backup-snapshot` job that exercises both backup snapshot fencing and the managed-storage ancestry-replacement read regression on a POSIX filesystem rather than leaving those paths Windows-only or source-assertion-only.
