...
- managed-storage path guards that reject symlinks and Windows junction/reparse entries below configured output/upload roots before generated files or uploaded media are created or resolved, and generated-output downloads that open a regular file once, compare the opened descriptor with the current managed path, then stream from that validated handle so a later pathname lookup cannot redirect the download
...
The linked managed-storage regression uses only temporary output/upload roots. It proves that ordinary managed IDs remain usable, static symlink/junction/reparse entries are rejected, generated-output reads return the intended regular file, and a descriptor that resolves to a different file object than the checked managed path is rejected before the download route can stream it.
