...
The same required category is run by `.github/workflows/safe-check.yml` as the Windows `safe-smoke` job and by `scripts/check_all.ps1` with no flags. CI also runs focused managed-storage, transcription-dispatch, question-identity, question-code-readiness, and project-flow-scope regressions directly where platform coverage matters. The Ubuntu `posix-backup-snapshot` job exercises backup snapshot fencing, managed-storage ancestry-replacement read/write behavior, write identity, the DB commit-window replacement race, providerless question identity/readiness, and project-flow-scope checks on a POSIX filesystem rather than leaving those paths Windows-only or source-assertion-only.
