...
The same required category is run by `.github/workflows/safe-check.yml` as the Windows `safe-smoke` job and by `scripts/check_all.ps1` with no flags. CI also runs focused managed-storage, question-identity, and question-code-readiness regressions directly on both Windows and Ubuntu. The Ubuntu `posix-backup-snapshot` job exercises backup snapshot fencing, managed-storage ancestry-replacement read/write behavior, write identity, the DB commit-window replacement race, and providerless question identity/readiness checks on a POSIX filesystem rather than leaving those paths Windows-only or source-assertion-only.
...
The question-identity regression uses only a temporary SQLite database and a stubbed in-process provider function, so no external AI request occurs. It proves same-flow duplicate insert/update and route writes are rejected while cross-flow reuse remains valid, exact `question_id` prevents evidence from leaking across flow versions that share the same code, integrated review stays inside persisted `source_flow_id`, ambiguous legacy integrated analyses fail closed, cross-participant generation rejects foreign-project questions before provider work, and model-returned question labels cannot widen canonical scope. `tests/smoke_question_code_readiness.py` separately proves a historical same-flow duplicate is surfaced as `duplicate_question_code` while a matching code in another flow is not. CI runs the readiness regression directly on both Windows and Ubuntu. The detailed contract is in `docs/question-identity.md`.
