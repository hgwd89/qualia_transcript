...
- production transcription dispatch fencing: direct-local and OpenAI-to-local paths preserve the durable lease/result-write callbacks, fallback reserves the canonical write before deleting partial OpenAI segments, stale attempts cannot start replacement provider work, and the dedicated regression is providerless
- transcription lifecycle write fencing: standalone preflight cleanup and pending-attempt creation are separately result-write-reserved; OpenAI/local `running` and `error` transitions are fenced; OpenAI raw evidence and long-audio manifests are published only while the durable attempt owns the write reservation
...
- semantic durable-job integrity: persisted semantic analysis uses `analyze_semantic`, requires a result-write lease before `AIAnalysis` commit, preserves `max_segments`/`no_ai` in durable `request_json` across retry and detached execution, rejects mismatched same-scope requests, reports an already-active same-scope CLI job as in-progress rather than success, preserves no-fragments diagnostics, supports current-attempt crash-window result recovery, and runs providerless on both Windows and Ubuntu in `.github/workflows/processing-jobs.yml`
- canonical AI service-boundary integrity: `tests/smoke_ai_result_service_guards.py` proves direct unguarded calls to per-question, participant-summary, cross-participant, integrated, and mapping save services are rejected before mocked provider work and leave canonical `AIAnalysis`/`UtteranceMapping` rows unchanged; the regression is providerless and runs on both Windows and Ubuntu in `.github/workflows/processing-jobs.yml`
- AI analysis generation-source provenance: `tests/smoke_analysis_source_provenance.py`, `tests/smoke_analysis_provenance_edge_cases.py`, and `tests/smoke_analysis_provenance_sqlite_parity.py` jointly prove persisted per-question, participant-summary, cross-participant, and integrated analyses bind to an `analysis-input-v1` SHA-256 fingerprint of the canonical provider inputs captured before provider work; provider prompts and fingerprints share the same deduplicated mapped-segment set even when historical duplicate mapping rows exist; provider-time drift is rejected before canonical result commit; stale drafts cannot be approved; stale or legacy approved rows become production-readiness blockers and cannot be formally exported; formal export records exact analysis IDs/provenance hashes; and the read-only SQLite readiness implementation produces hashes identical to the ORM implementation for all four formal analysis types. All three regressions are providerless and run on both Windows and Ubuntu in `.github/workflows/processing-jobs.yml`.
...
- question identity and evidence-scope integrity: non-empty `question_code` values are unique only within one interview flow, the same code remains valid across different flow versions/projects, SQLite startup installs non-destructive same-flow INSERT/UPDATE guards, route conflicts fail without mutation, question-specific approval uses canonical `AIAnalysis.question_id` rather than text-code coincidence, cross-participant generation validates question ownership before provider work and normalizes model labels, integrated analysis persists `source_flow_id`/`source_question_ids`/`source_interview_ids`, ambiguous legacy integrated review fails closed, and historical same-flow duplicates become `duplicate_question_code` readiness blockers without rewriting source research structure
- project-flow-scope integrity: a pipeline may fill a missing `Interview.flow_id` only when exactly one project flow exists, it re-resolves that condition after acquiring the result-write reservation, multi-flow ambiguity never rewrites interview provenance, cross-analysis exposes every configured flow explicitly, integrated analysis is currently single-flow and refuses ambiguous/incomplete source sets before provider work or durable job creation, and approval remains pinned to the generation-time interview set
...
- readiness detection of declared SQLite FK violations, explicit `ProcessingJob.question_id` orphans, same-flow duplicate `question_code` identities, and semantic durable-job scope violations whose referenced IDs still exist but belong to the wrong project/flow or violate the job type's required/forbidden interview/question shape
- readiness traceability regressions: question mappings are rejected when an interview has no assigned flow, every approved `source_segment_id` must individually support the finding's `evidence_quote`, every formally approved analysis must also have current `analysis-input-v1` generation provenance, and a retained raw transcript snapshot can satisfy a completed transcription only when its `transcription_id`, `interview_id`, and `created_at_utc` match the current `started_at`–`completed_at` generation and its filename is not already tombstoned to a deleted stable owner. Reusing the same SQLite project/interview/transcription integer IDs—even with identical generation timestamps—must not let a predecessor snapshot mask a missing replacement snapshot
...
The participant-code identity regression uses only a temporary legacy-style SQLite database whose pre-existing `participants` table has duplicate historical codes and no composite unique constraint. It proves startup preserves those rows, installs forward INSERT/UPDATE guards, rejects new duplicates, keeps create/edit conflicts non-destructive, allocates `Pxx` monotonically after deletion, blocks historical duplicates in global readiness, and keeps the blocker plus participant counts isolated to the owning project in project-scoped readiness. The detailed contract is in `docs/participant-identity.md`.

The question-identity regression uses only a temporary SQLite database and a stubbed in-process provider function, so no external AI request occurs. It proves same-flow duplicate insert/update and route writes are rejected while cross-flow reuse remains valid, exact `question_id` prevents evidence from leaking across flow versions that share the same code, integrated review stays inside persisted source provenance, ambiguous multi-flow integrated generation/review fails closed, cross-participant generation rejects foreign-project questions before provider work, and model-returned question labels cannot widen canonical scope. `tests/smoke_question_code_readiness.py` separately proves a historical same-flow duplicate is surfaced as `duplicate_question_code` while a matching code in another flow is not. CI runs the readiness regression directly on both Windows and Ubuntu. The detailed contract is in `docs/question-identity.md`.

The project-flow-scope regression uses only a temporary SQLite database and no external provider. It proves that multi-flow ambiguity does not mutate an unassigned interview, integrated scope failure occurs before durable job creation, serialized admission revalidates integrated scope, cross-analysis presents all configured flows, valid single-flow integrated analysis persists exact flow/question/interview provenance, incomplete participant interviews block provider work, and a later interview cannot retroactively become evidence for an older analysis. The pipeline also rechecks sole-flow assignment after acquiring its durable result-write reservation. The detailed contract is in `docs/project-flow-scope.md`.

The analysis-source provenance regressions use only temporary SQLite databases and in-process provider stubs. The generation regression captures the canonical source fingerprint before mocked provider work, deliberately mutates source state during provider execution to prove the result-write boundary rejects stale provider output, verifies post-generation mapping or participant-identity changes block later approval/export, verifies current approved output records exact validated provenance hashes, and confirms historical analyses without a generation fingerprint cannot become formally approved. The edge-case regression proves historical duplicate mapping rows cannot duplicate the prompt or create a prompt/fingerprint mismatch, and proves production-readiness blocks stale or provenance-less approved rows. The parity regression independently verifies that the read-only SQLite readiness implementation reproduces the ORM fingerprint for all four formal analysis types. None of these tests calls an external provider or touches the real research database/output tree.
...
This adds segment flags, speaker assignments, output flags, integrated no-AI checks, CLI guards, AI-analysis review/export, integrated preview checks, and deeper project-pipeline resilience/write-fencing checks. Those pipeline fixtures use explicit valid flow assignments unless the test is specifically exercising missing/ambiguous flow behavior. These checks must use temporary fixtures/directories where applicable.
...
Use temporary fixtures and output paths. Approval must require a current generation-time source fingerprint plus resolvable respondent evidence. Formal approved-analysis exports must remain approved-only, must revalidate every included analysis against current canonical inputs under a serialized database snapshot, and must record the validated analysis IDs/provenance hashes in `GeneratedFile.generation_params_json`. Production-readiness performs the same provenance check through a read-only SQLite implementation before declaring approved analyses formally deliverable.
