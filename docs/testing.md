...
The same required category is run by `.github/workflows/safe-check.yml` as the Windows `safe-smoke` job and by `scripts/check_all.ps1` with no flags. CI also runs a focused Ubuntu `posix-backup-snapshot` job so descriptor-relative ancestry fencing and extended-attribute rollback behavior are exercised on a POSIX filesystem rather than only skipped on Windows.
