...
- AI analysis generation-source provenance: `tests/smoke_analysis_source_provenance.py` proves persisted per-question, participant-summary, cross-participant, and integrated analyses bind to an `analysis-input-v1` SHA-256 fingerprint of the canonical provider inputs captured before provider work; the result is rejected if those inputs drift before the durable result-write commit, a draft cannot be approved after its generation inputs change, an approved analysis cannot be formally exported after its source becomes stale, formal export records the exact analysis IDs/provenance hashes it validated, and legacy analyses without generation provenance fail closed. The regression is providerless and runs on both Windows and Ubuntu in `.github/workflows/processing-jobs.yml`.
...
The analysis-source provenance regression uses only a temporary SQLite database and an in-process provider stub. It captures the canonical source fingerprint before mocked provider work, deliberately mutates source state during provider execution to prove the result-write boundary rejects stale provider output, verifies that post-generation mapping or participant-identity changes block later approval/export, verifies current approved output records the exact validated provenance hashes, and confirms historical analyses without the generation fingerprint cannot become formally approved. It never calls an external provider and never touches the real research database or output tree.
