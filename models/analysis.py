...
class AIAnalysis(db.Model):
    ...
    # per_question / per_participant / theme / integrated / cross_participant
    analysis_type = db.Column(db.Text, nullable=False)
    title         = db.Column(db.Text)
    summary_text  = db.Column(db.Text)
    # JSON: { findings: [{point, evidence_quote, source_segment_ids,
    #                     participant_codes, question_codes, confidence}],
    #         implications, unresolved }
    content_json  = db.Column(db.Text)
    model_used    = db.Column(db.Text)

    # Human review state. AI outputs are never treated as formally approved by default.
    # draft / approved / rejected
    review_status = db.Column(db.Text, nullable=False, default="draft")
