...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] safe smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] launcher runtime-config smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] generated-file integrity smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] media upload integrity smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] linked managed-storage guard smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] backup/restore integrity hardening smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] backup snapshot fencing smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] managed backup collector guard smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] backup atomic-publish smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] backup service-boundary smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] runtime maintenance exclusion smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] app-factory runtime exclusion smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] admin CLI maintenance exclusion smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] read-only runtime exclusion smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] durable job admission smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] project deletion lifecycle smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] raw snapshot provenance smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] project scope/delete guard smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] participant code identity smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
Write-Host "[PASS] participant code identity smoke checks passed."

Write-Host "[INFO] Running question code identity/evidence-scope smoke check..."
python tests/smoke_question_code_identity.py
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] question code identity/evidence-scope smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
Write-Host "[PASS] question code identity/evidence-scope smoke checks passed."

Write-Host "[INFO] Running interview creation scope smoke check..."
python tests/smoke_interview_create_scope.py
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] interview creation scope smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] role normalization/update integrity smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] SQLite foreign key smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] readiness FK orphan smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] readiness traceability smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] project-scoped readiness smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] resumed analysis UI smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { Write-Host "[FAIL] DPAPI secret-store smoke checks failed (exit code: $LASTEXITCODE)."; exit $LASTEXITCODE }
