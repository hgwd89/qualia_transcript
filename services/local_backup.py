...
def validate_backup(archive_path: str | os.PathLike) -> dict:
    """Validate paths, manifest, hashes and the backed-up SQLite database."""
    ...
    with tempfile.TemporaryDirectory(prefix="qualia_backup_validate_") as tmp:
        ...
        with zipfile.ZipFile(archive, "r") as zf:
            names = [_safe_member_name(info.filename) for info in zf.infolist() if not info.is_dir()]
        ...
        db_path = root / str(manifest.get("database_archive_path") or DB_ARCHIVE_PATH)
...
def restore_backup(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="qualia_restore_stage_") as tmp:
        ...
        # A restore must not run while the current DB is actively locked. The app
        # should be stopped before restore; this exclusive-lock probe catches the
        # most dangerous case and Windows file replacement adds another guard.
