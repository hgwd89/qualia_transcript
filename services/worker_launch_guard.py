...
def launch_job_or_preserve_active(
    ...
    pending on the same attempt/current-retry anchor, with no launch reservation
    or PID attached. Once `worker_pid` is non-null (including the 0 launch-
    reservation sentinel), or the child has advanced the job to running/succeeded,
    the parent must not write `failed`: subprocess creation may already have
    succeeded and the child owns the durable state transition.
    """
    job_id = int(job.id)
