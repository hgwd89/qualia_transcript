    ...
    with tempfile.TemporaryDirectory(prefix="qualia_pipeline_write_fencing_") as tmp:
        ...
        try:
            ...
            with app.app_context():
                ...
                db.session.flush()
                flow2 = InterviewFlow(project_id=project2.id, title="Flow")
                db.session.add(flow2)
                db.session.flush()
                interview2 = Interview(project_id=project2.id, flow_id=flow2.id, status="pending")
                ...
                db.session.flush()
                flow3 = InterviewFlow(project_id=project3.id, title="Flow")
                db.session.add(flow3)
                db.session.flush()
                interview3 = Interview(project_id=project3.id, flow_id=flow3.id, status="transcribed")
                ...
                db.session.flush()
                flow4 = InterviewFlow(project_id=project4.id, title="Flow")
                db.session.add(flow4)
                db.session.flush()
                interview4 = Interview(project_id=project4.id, flow_id=flow4.id, status="pending")
